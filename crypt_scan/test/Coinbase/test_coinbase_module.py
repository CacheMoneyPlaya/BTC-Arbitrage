...
class TestCoinbase():
    ...
    @classmethod
    def setup_class(cls):
        cls.coi_msg = {'type': 'l2update',
                       'product_id': 'BTC-USD',
                       'changes': [['buy',
                                    '40143.01',
                                    '0.26000000']],
                       'time': '2021-06-15T22:29:57.478518Z'}
    ...
    def test_can_get_quantity(self):
        assert self.coi_module.get_quantity(
        ) == self.coi_msg.get('changes')[0][2]

    def test_can_validate(self):
        assert self.coi_module.is_valid()
